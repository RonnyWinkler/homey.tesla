...
module.exports = class TeslaOAuth2App extends OAuth2App {
  ...
  static OAUTH2_DRIVERS = [ 'car', 'energy_site' ]; // Default: all drivers
  ...
  getOAuth2Client({
    ...
    this.log("getOAuth2Client() => client_id: ", client._clientId);
    return client;
  }
  ...
  createOAuth2Client({
    ...
    client._clientId = this.homey.settings.get('client_id') || '';
    client._clientSecret = this.homey.settings.get('client_secret') || '';
    this.log("createOAuth2Client() => client_id: ", client._clientId);
    return client;
  }
}
