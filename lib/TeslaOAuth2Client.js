...
const HTTP_TIMEOUT = 30000; // 30 sec Timeout
...
// Optional:
  ...
  async commandSigned(id, command, parameters){
    let base64String = this._getSignedBody(command, parameters);
  ...
  async _getSignedBody(command, parameters){
    return '';
  }
  ...
  async commandSetTemperature(id, driverTemperature, passengerTemperature) {
    let result = await this.post({
  ...
  async commandPreconditioning(id, state){
      let endpoint;
      if(state){
        endpoint = this.getToken().fleet_api_base_url+'/api/1/vehicles/'+id+'/command/auto_conditioning_start';
      } 
      else{
        endpoint = this.getToken().fleet_api_base_url+'/api/1/vehicles/'+id+'/command/auto_conditioning_stop';
      }
      let result = await this.post({
        path: endpoint
      });
  ...
  async commandPreconditioningMode(id, mode){
    let result = await this.post({
  ...
  async commandPreconditioningLevel(id, level){
    let cop_temp;
    if (level == 'low'){
      cop_temp = 1;
    }
  ...
  async commandChargePort(id, state){
    let endpoint;
    if(state){
      endpoint = this.getToken().fleet_api_base_url+'/api/1/vehicles/'+id+'/command/charge_port_door_open';
    } 
    else{
      endpoint = this.getToken().fleet_api_base_url+'/api/1/vehicles/'+id+'/command/charge_port_door_close';
    }
    let result = await this.post({
      path: endpoint
    });
  ...
  async commandChargeOn(id, state){
    let endpoint;
    if(state){
      endpoint = this.getToken().fleet_api_base_url+'/api/1/vehicles/'+id+'/command/charge_start';
    } 
    else{
      endpoint = this.getToken().fleet_api_base_url+'/api/1/vehicles/'+id+'/command/charge_stop';
    }
    let result = await this.post({
      path: endpoint
    });
