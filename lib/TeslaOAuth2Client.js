...
module.exports = class TeslaOAuth2Client extends OAuth2Client {
  ...
  static AUTHORIZATION_URL = 'https://auth.tesla.com/oauth2/v3/authorize?prompt_missing_scopes=true&prompt=login';
  static SCOPES = [ 
    'openid', 
    'offline_access', 
    'vehicle_device_data', 
  ...
  async setEnergySiteBackupReserve(id, backupReserve = 5) {    
    return await this.post({
      path: this.getToken().fleet_api_base_url + '/api/1/energy_sites/'+id+'/backup',
      headers: {},
      json:{
  ...
  async getEnergySiteHistoryDays(id) {    
    return await this.get({
      path: this.getToken().fleet_api_base_url + '/api/1/energy_sites/'+id+'/history?kind=energy&period=day'
    });
  }

  async getEnergySiteHistoryCalendar(id, startDate, endDate, timezone='UTC', period='day') {
    return await this.get({
      path: this.getToken().fleet_api_base_url + '/api/1/energy_sites/'+id+'/calendar_history?kind=energy&start_date='+encodeURIComponent(startDate)+'&end_date='+encodeURIComponent(endDate)+'&period='+period+'&time_zone='+timezone
    });
  }

  // FLEET TELEMETRY ENDPOINTS =======================================================================================
  async fleetTelemetryStatus(id) {    
    let result = await this.post({
      path: this.getToken().fleet_api_base_url +  '/api/1/vehicles/fleet_status',
      headers: {},
      json: {"vins": [ id ]}
  ...
  async fleetTelemetryConfig(id) {
    let result = await this.get({
      path: this.getToken().fleet_api_base_url + '/api/1/vehicles/'+id+'/fleet_telemetry_config',
    });
    return result;
  }

  async fleetTelemetryErrors(id) {
    let result = await this.get({
      path: this.getToken().fleet_api_base_url + '/api/1/vehicles/'+id+'/fleet_telemetry_errors',
    });
    return result;
  }

  async fleetTelemetryActivate(id, config, jws) {    
    let result = await this.post({
      path: this.getToken().fleet_api_base_url +  '/api/1/vehicles/fleet_telemetry_config',
      headers: {
        'X-Tesla-JWS-Signature': jws 
      },
      json: config
    });
    return result;
  ...
  async fleetTelemetryActivateJws(id, jws) {    
    let result = await this.post({
      path: this.getToken().fleet_api_base_url +  '/api/1/vehicles/fleet_telemetry_config_jws',
      json: {
        "vins": [ id ],
        "token": jws
      }
    ...
    async fleetTelemetryDeactivate(id) {    
