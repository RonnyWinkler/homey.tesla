...
// Optional:
  ...
  async commandPreconditioning(api, id, state){
      let endpoint;
      if(state){
  ...
  async commandPreconditioningMode(api, id, mode){
    let result = await this.post({
  ...
  async commandPreconditioningLevel(api, id, level){
    let cop_temp;
    if (level == 'low'){
      cop_temp = 1;
    }
  ...
  async commandDefrost(api, id, state){
    let result = await this.post({
  ...
  async commandScheduleCharging(api, id, action, hh, mm){
    let result = await this.post({
      path: this.getCommandUrl({
        host: this.getToken().fleet_api_base_url,
  ...
  async commandScheduleDeparture(api, id, action, hh, mm){
    let result = await this.post({
      path: this.getCommandUrl({
        host: this.getToken().fleet_api_base_url,
