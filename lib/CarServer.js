...
class CarServer {
    ...
    async #requestAction(action, domain) {
        ...
        this.api.log("CarServer.#requestAction() Signed message payload: "+JSON.stringify({ vehicleAction: action }));
        const payload = this.actionProto.create({ vehicleAction: action });
        // this.api.log("CarServer.#requestAction() Proto message payload:  ", payload);// JSON.stringify(payload));
